...
import { createOrder, fecthOrders, fetchAllOrders, fetchCount, updateOrder } from './orderAPI';
...
export const fecthOrdersAsync=createAsyncThunk(
  'order/fecthOrders',
  async ()=>{
    const response=await fecthOrders();
    return response.data;
  }
)
...
export const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fecthOrdersAsync.pending, (state) => {
        state.status = 'loading';
      })
      .addCase(fecthOrdersAsync.fulfilled, (state, action) => {
        state.status = 'idle';
